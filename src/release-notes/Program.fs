...
let private createRelease (config: ReleaseNotesConfig) (client: GitHubClient) =
    ...
    let unknownFiles =
        files |> List.filter (fun (found, f) -> not found) |> List.map (fun (_, f) -> f)

    if unknownFiles.Length > 0 then
        failwithf
            "The following files were not found and can not be read to include in the release body: %A"
            unknownFiles
    ...
    let body =
        let body =
            files
            |> List.fold
                (fun (state: StringBuilder) (found, f) -> state.AppendLine(File.ReadAllText(f)))
                (StringBuilder())
    ...
    | Some s ->
        ...
        client.Repository.Release.Edit(config.GitHub.Owner, config.GitHub.Repository, s.Id, ReleaseUpdate(Body = body))
        |> Async.AwaitTask
        |> Async.RunSynchronously
    | None ->
...
let private findCurrentAndNextVersion (config: ReleaseNotesConfig) (client: GitHubClient) versionQuery =
    ...
    let minVersion =
        ...
        | "master"
        | "main" -> SemVer.parse "0.0.1"
        | query when Regex.IsMatch(query, "\d+\.x") -> SemVer.parse <| query.Replace(".x", ".0") + ".0"
        | query when Regex.IsMatch(query, "\d+\.\d+") -> SemVer.parse <| query + ".0"
        | query when Regex.IsMatch(query, "\d+\.\d+.0") -> SemVer.parse <| query
        | _ -> failwithf "%s is not a valid version query" versionQuery
    ...
    let foundOldVersion =
        ...
        |> Seq.filter (fun v ->
            ...
            | "master"
            | "main" -> true
            | query when Regex.IsMatch(query, "\d+\.x") -> v.Major = minVersion.Major
            | query when Regex.IsMatch(query, "\d+\.\d+") -> v.Major = minVersion.Major && v.Minor = minVersion.Minor
            | query when Regex.IsMatch(query, "\d+\.\d+.0") -> v.Major = minVersion.Major && v.Minor = minVersion.Minor
            | _ -> false)
        |> Seq.sortByDescending (fun v -> v)
        |> Seq.tryHead
    ...
    | query when Regex.IsMatch(query, "^\d+\.\d+.0$") || Regex.IsMatch(query, "^\d+\.\d+$") ->
        let nextVersion =
...
let private writeMarkDownReleaseNotes (config: ReleaseNotesConfig) (client: GitHubClient) oldVersion =
    ...
    sprintf
        "### [View the full list of issues and PRs](%sissues?utf8=%%E2%%9C%%93&q=label%%3A%s)"
        config.GitHub.Url
        releasedLabel
    |> writer.WriteLine
    ...
    |> Option.iter (fun output ->
        ...
        let writeGenerated () =
            ...
            let groupedClosedIssues =
                ...
                |> Seq.sortBy (fun kv ->
                    if kv.Key = config.UncategorizedLabel then
                        -1
                    else
                        kv.Key.Length)
...
"""
        ...
        let currentPatchReleases =
            seq { 0 .. Convert.ToInt32(cv.Patch) }
            |> Seq.rev
...
"
        ...
        currentPatchReleases
        |> List.iter (fun (v: SemVerInfo) ->
...
let private writeReleaseNotes (config: ReleaseNotesConfig) (client: GitHubClient) oldVersion =
    ...
    | Markdown -> writeMarkDownReleaseNotes config client oldVersion |> ignore
    | AsciiDoc -> writeAsciiDocReleaseNotes config client oldVersion
...
let main argv =
    ...
    | Some p ->
        try
            ...
            let uncategorizedHeader =
                p.TryGetResult UncategorizedHeader |> Option.defaultValue uncategorizedLabel

            let oldVersionOnly =
                match p.TryGetSubCommand() with
                | Some FindPreviousVersion -> true
                | _ -> false

            let generateRelease =
                match p.TryGetSubCommand() with
                | Some (CreateRelease _) -> true
                | _ -> false

            let applyLabels =
                match p.TryGetSubCommand() with
                | Some ApplyLabels -> true
                | _ -> false
            ...
            let labels =
                ...
                [ (uncategorizedLabel, uncategorizedHeader) ] @ labels |> Map.ofList
